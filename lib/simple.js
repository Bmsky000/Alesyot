      ...
      exports.makeWASocket = (connectionOptions, options = {}) => {
          let client = (opts['legacy'] ? makeWALegacySocket : makeWASocket)(connectionOptions)

          // client.ws.on('CB:stream:error', (stream) => {
          //     const { code } = stream || {}
          //     if (code == '401') client.ev.emit('clientection.update', {
          //         clientection: 'logged Out',
          //         lastDisclientect: {
          //             error: {
          //                 output: {
          //                     statusCode: DisclientectReason.loggedOut
          //                 }
          //             },
          //             date: new Date()
          //         }
        ...
        /**
          ...
          */
          ...
          client.resize = async (buffer, uk1, uk2) => {
           ...
           */
           client.sendContact = async (jid, data, quoted, options) => {
              ...
              for (let [number, name] of data) {
                  ...
                  let biz = await client.getBusinessProfile(njid) || {}
                  // N:;${name.replace(/\n/g, '\\n').split(' ').reverse().join(';')};;;
                  let vcard = `
      BEGIN:VCARD
      VERSION:3.0
      ...
      PHOTO;BASE64:${(await client.getFile(await client.profilePictureUrl(njid)).catch(_ => ({})) || {}).data?.toString('base64')}
      X-WA-BIZ-DESCRIPTION:${(biz.description || '').replace(/\n/g, '\\n')}
      X-WA-BIZ-NAME:${(((client.chats[njid] || {}) || { vname: client.chats[njid]?.name }).vname || client.getName(njid) || name).replace(/\n/, '\\n')}
      `.trim() : ''}
      END:VCARD
      `.trim()
          ...
          client.reply = (jid, text = '', quoted, options) => {
              return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : client.sendMessage(jid, { ...options, text, mentions: client.parseMention(text) }, { quoted, ...options, mentions: client.parseMention(text) })
          }
          ...
          */
           ...
           * @param {Object} options 
           */
          client.sendButton = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
              if (buffer) try { buffer = (await client.getFile(buffer)).data } catch { buffer = null }
              let message = {
                  ...options,
             ...
             client.sendBut = async(jid, content, footer, button1, row1, quoted) => {
          const buttons = [
      ...
      const buttonMessage = {
          ...
          buttons: buttons,
          headerType: 1,
          mentions: client.parseMention(footer+content)
      }
      return await client.sendMessage(jid, buttonMessage, {quoted})
        }

         client.send2But = async(jid, content, footer, button1, row1, button2, row2, quoted) => {
          const buttons = [
           { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
      ...
      return await client.sendMessage(jid, buttonMessage, {quoted})
         ...
         client.send3But = async(jid, content, footer,button1, row1, button2, row2, button3, row3, quoted) => {
          const buttons = [
          { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
                { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
      ...
      return await client.sendMessage(jid, buttonMessage, {quoted})
        ...
        client.send4But = async(jid, content, footer,button1, row1, button2, row2, button3, row3, button4, row4, quoted) => {
          const buttons = [
          { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
              { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
              { buttonId: row3, buttonText: { displayText: button3 }, type: 1 },
      ...
      return await client.sendMessage(jid, buttonMessage, {quoted})
           ...
           * @param {Object} options 
           */
          client.sendButtonImg = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
              let type = await client.getFile(buffer)
              ...
              const buttonMessage = {
                  ...
                  caption: contentText,
                  footer: footerText,
                  mentions: await client.parseMention(contentText + footerText),
                  ...options,
                  buttons: buttons,
                  headerType: 4
              }

              return await client.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: 86400, contextInfo: { mentionedJid: client.parseMention(contentText + footerText) }, ...options })
          }
          client.send2ButtonImg = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
              let type = await client.getFile(buffer)
              let { res, data: file } = type
              if (res && res.status !== 200 || file.length <= 65536) {
              try { throw { json: JSON.parse(file.toString()) } }
          ...
          client.send3ButtonImg = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
              let type = await client.getFile(buffer)
              let { res, data: file } = type
              if (res && res.status !== 200 || file.length <= 65536) {
              try { throw { json: JSON.parse(file.toString()) } }
          ...
          client.sendH3Button = async (jid, content, displayText, link, displayCall, number, quickReplyText, id, quickReplyText2, id2, quickReplyText3, id3, quoted) => {
          let template = generateWAMessageFromContent(jid, proto.Message.fromObject({
           ...
           }), { userJid: client.user.jid, quoted: quoted});
          ...
          client.cMod = (jid, message, text = '', sender = client.user.jid, options = {}) => {
              ...
              client.sendHButtonLoc = async (jid, buffer, content, footer, distek, link1, quick1, id1,quoted) => {
          let template = generateWAMessageFromContent(jid, proto.Message.fromObject({
                     templateMessage: {
                   hydratedTemplate: {
                       hydratedContentText: content,
                       mentions: client.parseMention(content + footer),
                       locationMessage: { 
          ...
          mentions: client.parseMention(content + footer),
           ...
           }), { userJid: client.user.jid, quoted: quoted,     mentions: client.parseMention(content + footer)});
           return await client.relayMessage(
               jid,
               template.message,
               { messageId: template.key.id }
        ...
        client.sendHButt = async (jid, content, distek, link, discall, number, retek, id,quoted) => {
          let template = generateWAMessageFromContent(jid, proto.Message.fromObject({
                     templateMessage: {
                   hydratedTemplate: {
                       hydratedContentText: content,
        ...
        client.sendButtonLoc= async (jid, buffer, content, footer, button1, row1, quoted, options = {}) => {
        ...
        location: { jpegThumbnail: buffer },
          ...
          headerType: 6
            ...
            return await  client.sendMessage(jid, buttonMessage, {
                  quoted,
                  upload: client.waUploadToServer,
                  ...options
              })
        }
        client.send2ButtonLoc= async (jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) => {
          let buttons = [{buttonId: row1, buttonText: {displayText: button1}, type: 1},
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 }]
          let buttonMessage = {
        location: { jpegThumbnail: buffer },
          ...
          client.send3ButtonLoc= async (jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) => {
          let buttons = [{buttonId: row1, buttonText: {displayText: button1}, type: 1},
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
           { buttonId: row3, buttonText: { displayText: button3 }, type: 1 }
              ]
        ...
        location: { jpegThumbnail: buffer },
         ...
         /**
           ...
           * @param {Object} options 
           */
          client.sendButtonVid = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
              let type = await client.getFile(buffer)
              let { res, data: file } = type
              if (res && res.status !== 200 || file.length <= 65536) {
              try { throw { json: JSON.parse(file.toString()) } }
              ...
              const buttonMessage = {
                  ...
                  caption: contentText,
                  footer: footerText,
                  mentions: await client.parseMention(contentText),
                  ...options,
                  buttons: buttons,
                  headerType: 4
              }
          ...
          * @returns 
          */
          client.cMods = (jid, message, text = '', sender = client.user.jid, options = {}) => {
              let copy = message.toJSON()
              let mtype = Object.keys(copy.message)[0]
              let isEphemeral = false // mtype === 'ephemeralMessage'
              if (isEphemeral) {
